...
async function handleGetProducts(body, supabase) {
  try {
    ...
    const items = products.map(p => {
      // 所有 boolean 欄位在資料庫中都是字串 "true"/"false"
      const isBox = (p.is_box_preorder === 'true') ? 'Y' : 'N';
      ...
      return {
        ...
        category: p.category,
        isBoxPreorder: p.is_box_preorder === 'true',
        canDirectOrder: p.can_direct_order === 'true',
        isDirect: p.can_direct_order === 'true' ? 'Y' : 'N',
...
async function handleGetOrderInfo(body, supabase) {
  ...
  // 查詢團拆記錄（使用 break_records 資料表）
  let groupBreaks = await supabase.query('break_records', { eq: { user_id: userId }, order: { column: 'timestamp', ascending: false } });
...
async function handleAddOrder(body, supabase) {
  ...
  if (Array.isArray(products)) {
    products.forEach(p => {
      ...
      productMap.set(key, {
        ...
        fullPrice: parseFloat(p.threshold_price) || 0,
  ...
  const ordersToInsert = orderEntries.map(entry => {
    ...
    console.log('[ADD_ORDER] 處理訂單項目:', { itemName, cardNo, key, quantity, unitPrice });
    ...
    if (!productInfo) {
      ...
      console.error('[ADD_ORDER] 商品目錄中的 keys:', Array.from(productMap.keys()).slice(0, 10));
  ...
  // 🌟 檢查是否需要更新舊訂單價格（達到門檻時）
  // 這是原始 backend.js 的邏輯：達到門檻後，更新該用戶所有該商品的訂單價格
  try {
    for (const entry of orderEntries) {
      ...
      // 如果達到門檻，更新該用戶所有該商品的訂單價格
      if (totalAccumulated >= productInfo.threshold) {
        console.log('[ADD_ORDER] 🎯 達到門檻！開始更新 ' + nickname + ' 的所有 ' + itemName + ' 訂單價格 -> ' + productInfo.fullPrice);
        
        // 查詢該用戶該商品的所有訂單
        const userOrdersUrl = `${supabase.url}/rest/v1/orders?user_id=eq.${userId}&item=eq.${encodeURIComponent(itemName)}&card_no=eq.${encodeURIComponent(cardNo)}&select=id,quantity,unit_price,deposit`;
        const userOrdersResp = await fetch(userOrdersUrl, {
          headers: {
            'apikey': supabase.apiKey,
            'Authorization': `Bearer ${supabase.apiKey}`
          }
        ...
        if (Array.isArray(userOrders)) {
          for (const order of userOrders) {
            // 只更新價格不是門檻價的訂單
            if (Math.abs(order.unit_price - productInfo.fullPrice) > 0.01) {
  ...
  console.log('[ADD_ORDER] ✅ 訂單新增完成，共 ' + orderEntries.length + ' 筆');
...
async function handlePaymentNotification(body, supabase) {
  const { nickname, phone, paymentDetails } = body;
  ...
  await supabase.insert('order_history', [{
    user_id: userId,
    action: '付款通知',
...
async function handleCreateEcpayPayment(body, supabase) {
  const { orderDetails, totalAmount } = body;
...
async function handleCheckPaymentStatus(body, supabase) {
  ...
  const payment = await supabase.query('ecpay_records', { eq: { merchant_trade_no: merchantTradeNo } });
...
<body>
        ...
        <form id="psaForm">
            ...
            <div class="form-group">
                ...
                <div class="checkbox-container">
                    ...
                    <label class="checkbox-label"><input type="radio" name="shippingMethod" value="團拆直送" required onclick="calcPsaTotal()"> 團拆直送 (固定 880/張)</label>
                </div>
            </div>
    ...
    <div id="aboutPage" style="display:none;">
      ...
      <div style="max-width:800px;margin:0 auto;padding:20px;line-height:1.8;">
        ...
        <div style="background:#f8f9fa;padding:25px;border-radius:10px;margin-bottom:30px;border-left:5px solid var(--orange);box-shadow:0 2px 8px rgba(0,0,0,0.05);">
          <h3 style="color:var(--navy);margin-bottom:15px;font-size:20px;">🍀 求緣卡 (球員卡)</h3>
          <p style="color:#555;margin-bottom:0;">
  ...
  <script>
    ...
    function normalizeCardNo(value) {
      ...
      const match = text.match(/\d+/);
      if (match && match[0]) return String(parseInt(match[0], 10));
      return text;
    }
    ...
    function getEffectivePrice(item, additionalQty = 0) {
      ...
      if (threshold > 0 && discountPrice > 0) {
        const key = String(item.item || '').trim().toLowerCase() + '||' + String(item.cardNo || '').trim().toLowerCase();
        const currentAccum = globalAccum.get(key) || 0;
        // 計算購物車內相同商品的數量
        const cartQty = cart.reduce((sum, c) => {
          const cKey = String(c.item || '').trim().toLowerCase() + '||' + String(c.cardNo || '').trim().toLowerCase();
    ...
    async function callAPI(action, params = {}) {
      try {
        ...
        try {
          ...
          alert('伺服器回應錯誤: ' + text.substring(0, 100));
    ...
    async function doLogin() {
      ...
      showLoading('登入中...');
      const res = await callAPI('login', { phone, birthday: bday });
      hideLoading();
    ...
    async function loadCatalog() {
      ...
      try {
        ...
        if (res && res.success) {
          // 轉換 Supabase 資料格式為前端格式
          catalog = (res.items || []).map(p => {
            return {
              item: p.item,
              cardNo: p.cardNo,
              price: p.price,
              thresholdPrice: p.discountPrice || p.price,
              isOpen: p.status === 'open',
              isBox: p.isBox,
              isDirect: p.canDirectOrder ? 'Y' : 'N',
              stock: p.stock || 0,
              img1: p.images[0] || '',
              img2: p.images[1] || '',
              images: p.images || [],
              stockStatus: p.stockStatus,
              arrivalStatus: p.stockStatus,
              category: p.category || '其他',
              minGroup: p.minGroup || 0,
              closeTime: p.closeTime || '',
              threshold: p.threshold || 0,
              fullPrice: p.fullPrice || p.price,
              // 保留從後端取得的累積張數資訊
              accumulatedCount: p.accumulatedCount || 0,
              canDrawSp: p.canDrawSp,
              canDrawSignature: p.canDrawSignature,
              canDrawRelic: p.canDrawRelic,
              canDrawAutoRelic: p.canDrawAutoRelic,
              description: p.description || ''
            };
          });
          ...
          catalog.forEach(item => {
            if (item.isBox !== 'Y') { // 只有非卡盒商品才有累積張數
              const key = buildKey(item.item, item.cardNo);
              globalAccum.set(key, item.accumulatedCount || 0);
            }
          });
          
          console.log('[INIT] Initialized globalAccum with ' + globalAccum.size + ' items');
    ...
    function startGridTimer() {
      ...
      gridInterval = setInterval(() => {
        const timers = document.querySelectorAll('.grid-timer');
        if (timers.length === 0) return; // 如果沒有倒數計時器，直接返回
    ...
    function openProductDetail(item) {
      ...
      const key = buildKey(currentDetailItem.item, currentDetailItem.cardNo);
      ...
      const html = '<div style="display:flex;gap:30px;flex-wrap:wrap;">' +
        ...
        '<div style="flex:1;min-width:300px;">' +
          ...
          '<div id="accumDebug" style="margin-top:10px;padding:10px;border:1px dashed #ccc;border-radius:6px;font-size:12px;color:#666;display:none;"></div>' +
          priceHtml +
          groupProgressHtml +
          deadlineHtml +
          specialTagsHtml +
      ...
      // 取得後端累積除錯資料（可協助驗證全站累積）
      (async () => {
        try {
          const debugRes = await callAPI('debugAccum', { item: currentDetailItem.item, cardNo: currentDetailItem.cardNo });
          const debugEl = document.getElementById('accumDebug');
          if (debugEl && debugRes && debugRes.success) {
            debugEl.style.display = 'block';
            debugEl.innerText = '累積除錯：來源表=' + debugRes.table + ' | 累積=' + debugRes.accumulated + ' | 符合筆數=' + debugRes.matchedRows;
          }
        } catch (e) {
          console.warn('debugAccum failed', e);
        }
      })();
    ...
    async function checkoutCart() {
      ...
      try {
        const orderEntries = cart.map(item => ({
          ...
          totalFee: (item.price || 0) * (item.qty || 1),
          balanceAmount: (item.price || 0) * (item.qty || 1) // 初始餘額等於總額
        }));
    ...
    function displayOrders() {
      ...
      // 計算當前頁面所有訂單的總尾款
      const totalBalance = pageOrders.reduce((sum, o) => sum + Number(o.balance || 0), 0);
      ...
      pageOrders.forEach((o, idx) => {
        ...
        const originalIdx = user.orders.findIndex(order => 
          ...
          (order.total === o.total || order.totalFee === o.totalFee)
        );
        
        const balance = Number(o.balance || o.balanceAmount || 0);
        const total = Number(o.total || o.totalFee || 0);
        ...
        console.log('訂單:', o.item, o.cardNo, 'status:', statusText, 'balance:', balance, 'isCleared:', isCleared, 'isPendingPayment:', isPendingPayment);
        ...
        html += '<span class="order-info-value">NT$ ' + price.toLocaleString() + '</span>';
        html += '</div>';
        
        html += '<div class="order-info-item">';
        html += '<span class="order-info-label">總價</span>';
        html += '<span class="order-info-value">NT$ ' + total.toLocaleString() + '</span>';
        html += '</div>';
    ...
    function updateOrderSelection() {
      ...
      const count = selectedOrderIndices.size; // 使用全域計數
      let total = 0;
    ...
    async function submitPaymentNotification() {
      ...
      // 🌟 使用全域勾選狀態,不只是當前頁面的勾選框
      const breakChecked = Array.from(document.querySelectorAll('.break-checkbox:checked'));
      ...
      if (selectedOrderIndices.size > 0) {
        ...
        // 團拆付款
        isBreakPayment = true;
        const selectedBreaks = breakChecked.map(cb => {
          const idx = Number(cb.getAttribute('data-idx'));
          return user.groupBreaks[idx];
        }).filter(b => b);
    ...
    async function processEcpayPayment() {
      try {
        // 🌟 檢查是訂單付款還是團拆付款
        var orderChecked = Array.from(document.querySelectorAll(".order-checkbox:checked"));
        var breakChecked = Array.from(document.querySelectorAll(".break-checkbox:checked"));
    ...
    async function displayBreaks() {
      ...
      pageBreaks.forEach(function(b, idx) {
        ...
        if (isCleared) {
          ...
          html += '<input type="checkbox" class="break-checkbox" data-balance="' + balance + '" data-idx="' + originalIdx + '" data-id="' + breakId + '" onchange="updateBreakSelection()" onclick="event.stopPropagation();" style="width:18px;height:18px;cursor:pointer;margin-right:10px;">';
          html += '<div style="display:flex;flex-direction:column;gap:2px;">';
          html += '<span style="font-weight:600;color:white;">' + breakId + '</span>';
          html += '<span style="font-size:12px;color:rgba(255,255,255,0.85);">' + breakName + '</span>';
          html += '</div>';
        ...
        if (!isCleared) {
          ...
          html += '<span style="color:#e65100;font-size:20px;font-weight:bold;">NT$ ' + balance.toLocaleString() + '</span>';
          html += '</div>';
          html += '</div>';
        }
