/**
 * Cloudflare Worker - Ning's Card Store (Supabase 版本)
 * ✅ 使用 Supabase 資料庫 + Cloudflare Workers API
 * ✅ 包含: 登入、訂單查詢、商品詳情、團拆、PSA 鑑定
 */
...
<body>
  ...
  <script>
    // ===== API 設定 =====
    const API_URL = 'https://supabase.cnkuoc.workers.dev';
    ...
    async function doLogin() {
      ...
      showLoading('登入中...');
      const res = await callAPI('login', { phone, password: bday });
      hideLoading();
    ...
    async function loadCatalog() {
      ...
      try {
        ...
        if (res && res.success) {
          ...
          catalog = (res.products || []).map(p => {
            const images = [];
            if (p.imageUrl1) images.push(p.imageUrl1);
            ...
            return {
              ...
              cardNo: p.cardNo,
              price: p.price,
              thresholdPrice: p.thresholdPrice || p.price,
              isOpen: p.isAvailable === 'Y',  // 修正：字串比較
              isBox: p.category === '卡盒' ? 'Y' : 'N',
              isDirect: p.stockStatus === 'V' ? 'Y' : 'N',  // V = 現貨
              stock: p.remainingStock || 0,
              img1: p.imageUrl1,
              img2: p.imageUrl2,
              images: images,  // 加入 images 陣列
              stockStatus: p.stockStatus,
              arrivalStatus: p.stockStatus,  // 使用 stockStatus 作為 arrivalStatus
              category: p.category,
              minGroup: p.minGroupSize || 0,  // 加入開團最低張數
              closeTime: p.closeTime || '',  // 加入截止時間
              threshold: p.discountThreshold || 0  // 折扣門檻
            };
          });
      ...
      try {
        ...
        if (responseText.trim().toLowerCase().startsWith('<!doctype') ||
          ...
          return new Response(JSON.stringify({
            success: false,
            message: 'GAS 部署錯誤，請檢查。在 GAS 編輯器中：部署 → 管理部署 → 編輯 → 新版本 → 部署'
          }), {
            status: 502,
            headers: {
