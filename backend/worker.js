...
class SupabaseClient {
  ...
  async insert(table, data) {
    ...
    // 🔧 修正：檢查回應是否成功，處理非 JSON 錯誤
    if (!response.ok) {
      const contentType = response.headers.get('content-type');
      if (contentType && contentType.includes('application/json')) {
        const error = await response.json();
        console.error('[INSERT] Supabase 錯誤:', error);
        throw new Error(error.message || `Insert failed: ${response.statusText}`);
      } else {
        const errorText = await response.text();
        console.error('[INSERT] Supabase 錯誤 (非JSON):', errorText);
...
/**
 * 🕐 取得台灣時間 (UTC+8)
 * @returns {Date} 台灣時間的 Date 物件
 */
function getTaiwanTime() {
  const now = new Date();
  const taiwanOffset = 8 * 60; // 台灣 UTC+8，分鐘數
  return new Date(now.getTime() + taiwanOffset * 60 * 1000);
}

/**
 * 🕐 取得台灣時間的 ISO 字串
 * @returns {string} 台灣時間的 ISO 格式字串
 */
...
// ==================== 💳 綠界金流設定 (正式環境) ====================
const ECPAY_CONFIG = {
  MerchantID: '3479617',                 // 正式環境特店編號
  HashKey: 'iN7DSsQl9AwHZgzv',           // 正式環境 HashKey
  HashIV: 'FjgpEgbzEzUVHgTW',            // 正式環境 HashIV
...
async function handleLogin(body, supabase) {
  ...
  if (!Array.isArray(users) || users.length === 0) {
    ...
    return { success: false, message: '電話號碼或生日錯誤' }; // 不透露具體是哪個錯誤
  }
...
async function handleRegisterUser(body, supabase) {
  try {
    ...
    // 驗證手機號碼格式（0 開頭的 10 位數字）
    if (!/^0\d{9}$/.test(phone)) {

      return { success: false, message: '手機號碼格式不正確，請輸入 0 開頭的 10 位數字' };
    }
    ...
    // 🔧 修正：address 是門市名稱，應該存到 cvs_store_name
    const newUser = [{
      phone: phone,
      nickname: nickname || '',
      birthday: birthday || '',
      email: email || '',
      real_name: realName || '',
      cvs_store_name: address || '',      // 🔑 門市名稱
      cvs_store_id: storeNumber || ''     // 🔑 門市編號
    }];
    ...
    return { success: false, message: '註冊過程發生錯誤: ' + error.message };
  }
}
...
async function handleGetOrderInfo(body, supabase) {
  ...
  if (!birthday) {
    ...
    return { success: false, message: '請提供生日進行驗證' };
  }
  
  const users = await supabase.query('users', { eq: { phone } });
  if (!Array.isArray(users) || users.length === 0) {

    return { success: false, message: '電話號碼或生日錯誤' };
  }
  ...
  const formattedBreaks = Array.isArray(groupBreaks) ? groupBreaks.map(b => {
    // 🔥 移除 category 中的所有 emoji 和特殊字符，只保留中文文字
    let cleanCategory = (b.category || '棒球')
      .replace(/[\u{1F300}-\u{1F9FF}]/gu, '') // 移除 emoji (Unicode 範圍)
      .replace(/[⚾🏀🎯]/g, '') // 移除特定 emoji
      .replace(/[^\p{L}\p{N}]/gu, '') // 只保留字母和數字
      .trim() || '棒球';
  ...
  return { 
    success: true,
    id: users[0].id,  // 🔥 添加 user_id，前端支付時會用到
...
async function handleAddOrder(body, supabase) {
  ...
  const now = getTaiwanTimeISO(); // 🔧 使用台灣時間
...
請盡快處理此訂單。
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: 'Ning\'s Card <no-reply@ningscard.com>',
      to: [merchantEmail],
...
async function sendPaymentNotificationEmail(nickname, phone, paymentItems) {
  ...
  paymentItems.forEach((item, idx) => {
    ...
    paymentDetails += `   付款金額: NT$ ${item.amount}\n`;
    if (item.accountLast5) {
      paymentDetails += `   帳號後五碼: ${item.accountLast5}\n`;
    }
    if (item.date) {
      paymentDetails += `   付款日期: ${item.date}\n`;
    }
    if (item.note) {
      paymentDetails += `   備註: ${item.note}\n`;
    }
    paymentDetails += `\n`;
...
async function handleNotifyPaymentBulk(body, supabase) {
  ...
  try {
    ...
    console.error('[PAYMENT_NOTIFY] ❌ Email 通知發送失敗:', emailError);
  }
...
async function handlePaymentNotification(body, supabase) {
  ...
  // 📧 寄送 email 通知商家
  try {
    // 組裝付款項目資料以符合 sendPaymentNotificationEmail 的格式
    const paymentItems = [{
      item: item,
      amount: amount,
      accountLast5: key,
...
async function handleUseBreakCredit(body, supabase) {
  ...
  for (const credit of credits) {
    ...
    // 🌟 合併 breakIds 並記錄金額 (格式: "breakId@@breakName::amount")
...
async function handleGetAllBreakCredits(body, supabase) {
  ...
  if (!response.ok) {
    return { success: false, message: '查詢失敗', credits: [] };
  }
  
  // 轉換資料格式，將users.nickname提取到頂層
  const credits = data.map(item => ({
    id: item.id,
    user_id: item.user_id,
    nickname: item.users?.nickname || '未知',
...
async function handleAddBreakCredit(body, supabase) {
  ...
  if (!nickname || !credit || credit <= 0) {
    return { success: false, message: '請提供有效的暱稱和金額' };
  }
  ...
  if (!users || users.length === 0) {
    return { success: false, message: '找不到該暱稱的用戶' };
  }
  ...
  if (!insertResp.ok) {
    ...
    return { success: false, message: '新增失敗: ' + error };
  }
  
  return { success: true, message: '團拆金新增成功' };
}
...
async function handleBatchAddBreakCredit(body, supabase) {
  ...
  if (!nicknames || !Array.isArray(nicknames) || nicknames.length === 0) {
    return { success: false, message: '請提供暱稱列表' };
  }
  
  if (!credit || credit <= 0) {
    return { success: false, message: '請提供有效的金額' };
  }
  ...
  for (const nickname of nicknames) {
    try {
      ...
      if (!users || users.length === 0) {
        failCount++;
        failedNicknames.push(nickname + '(找不到用戶)');
        continue;
      }
      ...
      if (insertResp.ok) {
        ...
        failCount++;
        failedNicknames.push(nickname + '(新增失敗)');
      }
    } catch (error) {
      failCount++;
      failedNicknames.push(nickname + '(錯誤: ' + error.message + ')');
    }
  }
  
  return {
    success: true,
    message: `批量新增完成: 成功 ${successCount} 個，失敗 ${failCount} 個`,
    successCount,
    failCount,
    failedNicknames
  };
...
async function handleUpdateBreakCredit(body, supabase) {
  ...
  if (!id || !credit || credit <= 0) {
    return { success: false, message: '請提供有效的ID和金額' };
  }
  ...
  if (!updateResp.ok) {
    ...
    return { success: false, message: '更新失敗: ' + error };
  }
  
  return { success: true, message: '團拆金更新成功' };
}
...
async function handleDeleteBreakCredit(body, supabase) {
  ...
  if (!id) {
    return { success: false, message: '請提供有效的ID' };
  }
  ...
  if (!Array.isArray(records) || records.length === 0) {
    return { success: false, message: '找不到該團拆金記錄' };
  }
  ...
  if (!deleteResp.ok) {
    ...
    console.error('[deleteBreakCredit] 刪除失敗:', error);
    return { success: false, message: '刪除失敗: ' + error };
  }
  
  return { success: true, message: '團拆金刪除成功' };
}
...
async function handleCheckDailyFortune(body, supabase) {
  ...
  // 建立台灣時間的今天 00:00 和明天 00:00 (UTC 時間)
  ...
  // 查詢今天是否有抽籤記錄 (使用 result 和 draw_date 欄位)
  ...
  if (!fortuneResp.ok) {
    ...
    console.error('❌ 查詢失敗:', fortuneResp.status, errorText);
    return { success: false, hasDrawn: false };
  }
...
async function handleSaveDailyFortune(body, supabase) {
  ...
  // 使用新的表結構：result (運勢結果), draw_date (抽籤時間), status (狀態)
  const lotteryRecord = {
    user_id: userId,
    result: result,
    draw_date: taiwanTime.toISOString(),  // 使用台灣時間
    status: '已完成'
  };
  ...
  try {
    ...
    // 檢查是否有錯誤
    if (insertResult && insertResult.error) {
      console.error('❌ 插入時發生錯誤:', insertResult.error);
      return { success: false, message: '儲存失敗: ' + insertResult.error.message };
    }
...
async function handleCreateShipmentRecord(body, supabase) {
  ...
  const shipment = [{
    ...
    shipment_date: getTaiwanTimeISO(), // 🔧 使用台灣時間
...
async function handleGetShipmentRecords(body, supabase) {
  ...
  const records = (shipments || []).map(s => {
    // 處理 items 格式：如果是陣列轉成換行字串，如果是分號分隔轉成換行
    ...
    return {
      shipmentNumber: s.shipment_no || '',  // 前端期望 shipmentNumber
      shipmentNo: s.shipment_no || '',       // 保留向後兼容
      trackingNumber: s.tracking_no || '',   // 前端期望 trackingNumber
      items: itemsStr || '',
      status: s.status || '',
      shipmentDate: s.shipment_date || '',
      shipStore: s.ship_store || '',         // 711 店名
      storeNumber: s.store_number || '',     // 店號
      note: s.remark || ''                   // 備註欄位
    };
  });
  ...
  return {
    success: true,
    records: records  // 前端期望 records 欄位
  };
}
...
async function handleUpdateBreakStatusToPending(body, supabase) {
  ...
  try {
    ...
    for (const detail of details) {
      ...
      if (!breakId) {
        console.warn('[UPDATE_BREAK] ⚠️ 缺少 breakId');
        matchDetails.push({ breakId, nickname, matched: false, reason: '缺少 breakId' });
        continue;
      }
      
      // 🔥 優先使用 userId 直接匹配（最可靠的方法）
      let userId = detailUserId;
      
      if (!userId) {
        console.error('[UPDATE_BREAK] ❌ userId 為空，無法進行精確匹配!');
        console.error('[UPDATE_BREAK] detail:', JSON.stringify(detail));
        matchDetails.push({ breakId, nickname, matched: false, reason: 'userId 為空 - 前端未傳遞' });
        continue;
      }

      let phoneQueryResult = null;
      
      // 直接用 userId + breakId 進行精確查詢
      ...
      if (!Array.isArray(breaks) || breaks.length === 0) {
        console.warn('[UPDATE_BREAK] ❌ 用戶+break_id 精確匹配失敗:', { breakId, userId, nickname });
        ...
        // 🔍 再查詢：所有該 break_id 的團拆（不限 user_id）
        ...
        // ❌ 不再嘗試備用方案（會更新到錯誤的用戶）
        // 前端應該確保 userId 被正確傳遞
        matchDetails.push({ breakId, nickname, matched: false, reason: '用戶+break_id 精確匹配失敗 - 前端應傳遞正確的 userId' });
        continue;
      }
      ...
      if (!updateResp.ok) {
        ...
        console.error('[UPDATE_BREAK] HTTP 錯誤:', updateResp.status, errText);
  ...
  try {
    if (paymentType === 'order') {
      ...
      for (const detail of orderDetails) {
        ...
        if (!breakId) {
          console.warn('[UPDATE_BREAK_FAILED] ⚠️ 團拆缺少 breakId');
          continue;
        }
        ...
        if (!userId) {
          console.error('[UPDATE_BREAK_FAILED] ❌ userId 為空，無法進行精確匹配!');
        ...
        if (Array.isArray(breaks) && breaks.length > 0) {
          ...
          console.error('[UPDATE_BREAK_FAILED] ❌ 用戶+break_id 精確匹配失敗:', { breakId, userId, nickname });
        }
        
        if (!breakRecord) {
          console.error('[UPDATE_BREAK_FAILED] ❌ 找不到團拆 ' + breakId);
          continue;
        }
        ...
        if (patchResp.ok) {
          ...
          console.error('[UPDATE_BREAK_FAILED] ❌ 更新團拆 ' + breakId + ' 失敗: HTTP ' + patchResp.status);
        }
      }
    }
...
async function handleSearchOrders(body, supabase) {
  ...
  try {
    ...
    // 🔑 無論什麼搜尋條件，都要 JOIN 用戶資料，以顯示訂購者資訊
    // 查詢所有用戶
...
async function handleUpdateUser(body, supabase) {
  ...
  if (response.ok) {
    ...
    try {
      ...
      errorMsg = '更新失敗: ' + (result.message || result.details || text);
    } catch (e) {
      errorMsg = '更新失敗: ' + text;
    }
...
async function handleAddProduct(body, supabase) {
  ...
  if (response.ok) {
    const result = await response.json();
    return { success: true, message: '商品新增成功', product: result[0] };
  } else {
    const errorText = await response.text();
    return { success: false, message: '新增失敗: ' + errorText };
  }
}
...
async function handleGetAllProducts(body, supabase) {
  ...
  if (response.ok) {
    ...
    const errorText = await response.text();
    return { success: false, message: '查詢失敗: ' + errorText };
  }
}
...
async function handleUpdateProduct(body, supabase) {
  ...
  if (response.ok) {
    // 如果更新了商品到貨狀態為 'V'，連動更新相關訂單的到貨狀態
    if (updateData.stock_status === 'V') {
      try {
        // 先查詢商品的 item_name 和 card_no
    ...
    const errorText = await response.text();
    return { success: false, message: '更新失敗: ' + errorText };
  }
}
...
async function handleDeleteProduct(body, supabase) {
  ...
  if (response.ok) {
    ...
    const errorText = await response.text();
    return { success: false, message: '刪除失敗: ' + errorText };
  }
}
...
async function handleUpdateBreak(body, supabase) {
  const { id, status, paid, item, is_opened, is_shipped, user_id } = body;
      ...
      try {
        ...
        if (matchedUser) {
          ...
          errors.push({
            buyer,
            breakNumber,
            error: `找不到訂購人 "${buyer}"。可用的使用者: ${availableUsers || '(無)'}`
          });
        ...
        // break_id 就是團拆編號本身，所有訂購人共用同一個編號
        // 每筆記錄由 UUID 的 id 欄位唯一識別
        const breakId = breakNumber;
        
        // 準備新增的資料
        // 根據 Supabase breaks 表的欄位定義
        // ⚠️ balance 是 generated column，不能插入值，由系統自動計算
        const newBreak = {
          break_id: breakId,
          user_id: userId,
          name: breakName,
...
async function handleDeleteBreak(body, supabase) {
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('[deleteBreak] HTTP 錯誤:', response.status, errorText);
      return { success: false, message: '刪除失敗: ' + response.statusText };
    }
    ...
    console.error('[deleteBreak] 錯誤:', error);
...
async function handleDeleteOrder(body, supabase) {
  ...
  if (!id) {
    return { success: false, message: '缺少訂單 ID' };
  }
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('[deleteOrder] HTTP 錯誤:', response.status, errorText);
      return { success: false, message: '刪除失敗: ' + response.statusText };
    }
...
async function handleGetAllOrders(body, supabase) {
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('[getAllOrders] HTTP 錯誤:', response.status, errorText);
      return { success: false, message: '查詢失敗: ' + response.statusText };
    }
    ...
    const enrichedOrders = Array.isArray(orders) ? orders.map(o => ({
      ...o,
      nickname: userMap.get(o.user_id)?.nickname || '未知',
      phone: userMap.get(o.user_id)?.phone || '未知'
    })) : [];
    ...
    console.error('[getAllOrders] 錯誤:', error);
    return { success: false, message: '查詢失敗: ' + error.message };
  }
}
...
async function handleEcpayCallback(params, supabase) {
  try {
    ...
    // 🔥 冪等性檢查：如果已經處理過（狀態是 success），直接返回成功，不重複處理
    if (record.status === 'success' && rtnCode === '1') {

      return { success: true, message: '1|OK（已處理過）' };
    }
    ...
    if (rtnCode === '1') {
      ...
      try {
        ...
        if (paymentType === 'order') {
          ...
          // 團拆付款：根據 orderDetails 中每筆的實際金額更新

          for (const detail of orderDetailsArray) {
            const breakId = detail.breakId;
            const payAmount = Number(detail.balance || 0); // 🔥 使用 orderDetails 中記錄的實際金額
            const usedCredit = Number(detail.usedCredit || 0); // 🌟 前端傳來的團拆金金額
            const totalFeeFromDetail = Number(detail.totalFee || 0); // 🌟 總團費
            ...
            if (Array.isArray(matchingBreaks) && matchingBreaks.length > 0) {
              ...
              console.warn('[ECPay] ⚠️ 找不到團拆記錄:', breakId, '用戶:', record.user_id);
            }
          }
        }
      } catch (updateErr) {
...
async function handleGenerateShippingReport(body, supabase) {
  try {
    // 查詢所有未出貨的訂單和團拆
    // 🔧 使用 or 條件：is_shipped 為 null、false 或 'false' 都算未出貨
    ...
    if (Array.isArray(orders)) {
      for (const order of orders) {
        ...
        // 🔍 檢查訂單的到貨狀態
        // 下單時已經從商品目錄同步 arrival_status，直接使用即可
    ...
    userMap.forEach(user => {
      ...
      user.allOrdersArrived = allOrdersArrived; // 記錄此狀態供前端顯示
    ...
    return { success: false, message: '生成出貨報表失敗: ' + error.message };
  }
}
...
async function handleCreateShipment(body, supabase) {
  try {
    ...
    if (!insertRes.ok) {
      ...
      return { success: false, message: '建立出貨紀錄失敗: ' + errorText };
    }
    ...
    return { success: false, message: '建立出貨紀錄失敗: ' + error.message };
  }
}
...
async function handleGetAllShipments(body, supabase) {
  try {
    ...
    if (response.ok) {
      ...
      const errorText = await response.text();
      return { success: false, message: '查詢出貨紀錄失敗: ' + errorText };
    }
  } catch (error) {
    console.error('[GET_ALL_SHIPMENTS] Error:', error);
    return { success: false, message: '查詢出貨紀錄失敗: ' + error.message };
  }
}
...
async function handleBatchUpdateTrackingNumbers(body, supabase) {
  try {
    ...
    if (!Array.isArray(updates) || updates.length === 0) {
      return { success: false, message: '未提供更新資料' };
    }
    ...
    for (const update of updates) {
      ...
      if (!id) {
        results.push({ id, success: false, message: '缺少出貨紀錄 ID' });
        continue;
      }
      ...
      if (response.ok) {
        ...
        if (trackingNo && shipmentData && shipmentData.users && shipmentData.users.email) {
          try {
            ...
            console.error('[BATCH_UPDATE_TRACKING] ❌ Email 寄送失敗:', emailError);
            // 不影響更新結果
          }
        } else {
    ...
    return {
      success: true,
      message: `成功更新 ${successCount} 筆，失敗 ${failCount} 筆`,
      results
    };
  } catch (error) {
    console.error('[BATCH_UPDATE_TRACKING] Error:', error);
    return { success: false, message: '批次更新失敗: ' + error.message };
  }
}
...
async function handleDeleteShipment(body, supabase) {
  try {
    ...
    if (!shipmentId) {
      return { success: false, message: '缺少出貨紀錄 ID' };
    }
    ...
    if (response.ok) {
      return { success: true, message: '出貨紀錄已刪除' };
    } else {
      const errorText = await response.text();
      return { success: false, message: '刪除出貨紀錄失敗: ' + errorText };
    }
  } catch (error) {
    console.error('[DELETE_SHIPMENT] Error:', error);
    return { success: false, message: '刪除出貨紀錄失敗: ' + error.message };
  }
}
